...
For the human-based speech quality prediction, it generally aims to predict mean-opinion-scores from several different listeners. Nowdays, there are several benchmark dataset that are interestingly designed for different speech processing applicattions, For example, fo speech enhancement task, the speech asessment model aims to predict to quality of enhanced speech which has perfomed noise reduction to remove the noise parts.
